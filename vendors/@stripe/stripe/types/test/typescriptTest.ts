...
// @ts-expect-error ignore specific apiVersion.
stripe = new Stripe('sk_test_123', {
  apiVersion: '2019-11-05',
});

// @ts-expect-error ignore default apiVersion.
stripe = new Stripe('sk_test_123', {
  apiVersion: null,
});
...
(async (): Promise<void> => {
  ...
  const v2FAList: Stripe.V2.ApiList<Stripe.V2.FinancialAccount> = await stripe.v2.financialAccounts.list();
...
(async () => {
  const event = await stripe.v2.core.events.retrieve('event_123');
  if (
    event.type == 'account.configuration_recipient_data.feature_status_updated'
  ) {
    const eventData: Stripe.Events.AccountConfigurationRecipientDataFeatureStatusUpdatedEvent.Data =
      event.data;
    console.log(eventData.feature_name);

    // TODO (jar): Add this once we implement fetchRelatedObject()
    // const obj = (await event.fetchRelatedObject()) as Stripe.V2.Account;
  }
})();
